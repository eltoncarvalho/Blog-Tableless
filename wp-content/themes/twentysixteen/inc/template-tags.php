...
function twentysixteen_entry_meta() {
	...
	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( __( 'Comentar <span class="screen-reader-text"> on %s</span>', 'twentysixteen' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;
...
function twentysixteen_entry_taxonomies() {
	...
	if ( $categories_list && twentysixteen_categorized_blog() ) {
		printf( 'Categoria: <span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
